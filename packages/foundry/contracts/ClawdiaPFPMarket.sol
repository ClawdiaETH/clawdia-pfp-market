...
/**
 * @title ClawdiaPFPMarket
 * @notice Prediction market for picking Clawdia's wifey PFP (feminine remix of hubby's PFP).
 *         Users submit image URLs + stake $CLAWDIA. Others can stake on
 ...
 */
contract ClawdiaPFPMarket is ReentrancyGuard {
    ...
    constructor(address _clawdiaToken, uint256 _duration, address _admin, uint256 _stakeAmount) {
        clawdiaToken = IERC20(_clawdiaToken);
    ...
    function submit(string calldata imageUrl) external beforeDeadline nonReentrant {
        ...
        // Pull tokens
        clawdiaToken.safeTransferFrom(msg.sender, address(this), STAKE_AMOUNT);
    ...
    function claim() external nonReentrant {
        ...
        clawdiaToken.safeTransfer(msg.sender, payout);
    ...
    function banAndSlash(uint256 id) external onlyAdmin nonReentrant {
        ...
        clawdiaToken.safeTransfer(BURN_ADDRESS, slashAmount);
    ...
    function pickWinner(uint256 id) external onlyAdmin afterDeadline nonReentrant {
        ...
        // 1. Burn 25%
        clawdiaToken.safeTransfer(BURN_ADDRESS, burnAmount);
        emit Payout(BURN_ADDRESS, burnAmount, "burn");

        // 2. OP bonus 10%
        clawdiaToken.safeTransfer(winner.submitter, opBonus);
